...
def load_config(
    ...
    camera = CameraConfig(
        make=str(camera_data.get("make", "basler")).lower(),
    ...
    preview = PreviewConfig(
        ...
        max_fps=float(preview_data.get("max_fps", 30.0)),
        width=int(preview_data.get("width", 800)),
...
def validate_config(cfg: PyCamRecConfig) -> None:
    if cfg.camera.make != "basler":
        raise ValueError("Phase 1 supports only Basler cameras.")
    ...
    if cfg.recording_profile.id != "custom" and cfg.writer.container.lower() != "avi":
        raise ValueError("Built-in recording profiles must write AVI segment files.")
